...
class Chatbot:
    def __init__(self, window):
        ...
        self.message_session.config(
            width=45, height=15, bg="#4a4a4a", fg="white", state='disabled')
        ...
        self.send_button = Button(window, text='send', fg='white', bg='#2e5a9e', width=9, font=(
            'Helvetica', 12), relief='flat', command=self.reply_to_you)
    ...
    def reply_to_you(self, event=None):
        ...
        if close_match:
            reply = 'GVK: ' + self.Brain[close_match[0]][0] + '\n'
        else:
            reply = 'GVK: ' + 'Can"t find it in my knowledge base\n'
